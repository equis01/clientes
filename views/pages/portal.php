<?php if(session_status()!==PHP_SESSION_ACTIVE){ session_start(); } if(!isset($_SESSION['user'])){header('Location: /login');exit;} $client=isset($_SESSION['client_name'])?$_SESSION['client_name']:$_SESSION['user']; $folder=isset($_SESSION['folder_url'])?$_SESSION['folder_url']:'#'; require_once __DIR__.'/../../lib/gas.php'; $emailsList=[]; $userRec=gas_users(isset($_SESSION['user'])?$_SESSION['user']:null); if($userRec['ok'] && is_array($userRec['user'])){ $raw=isset($userRec['user']['email'])?$userRec['user']['email']:''; $parts=preg_split('/[;,]+/',$raw); if(is_array($parts)){ foreach($parts as $p){ $t=trim((string)$p); if($t!==''){ $emailsList[]=$t; } } } } ?>
<!DOCTYPE html>
<html lang="es">
<head>
